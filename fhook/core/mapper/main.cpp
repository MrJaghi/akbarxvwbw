...
bool IsDriverLoaded() {
	// Check if the driver device handle can be opened
	// This GUID MUST match the one in driver/main.c
...
int wmain(const int argc, wchar_t** argv) {
	...
	if (IsDriverLoaded()) {
		kdmLog(L"[+] Driver is already loaded. Skipping mapping." << std::endl);
		goto waitForGame;
	}
	...
	kdmLog(L"[+] Driver mapped successfully!" << std::endl);

waitForGame:
	kdmLog(L"[+] Waiting for game..." << std::endl);
	...
	while (true)
	{
		...
		{
			kdmLog(L"[+] Game found! Closing in 3 seconds..." << std::endl);
			Sleep(3000);
			break;
		}
		Sleep(1000);
