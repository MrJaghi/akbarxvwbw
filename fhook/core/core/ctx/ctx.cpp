...
bool unknowncheats::interface_t::initialize( ) {
	...
	if ( client_dll == 0 ) {
		printf( "[unknowncheats] client.dll not found. Waiting...\n" );
		return false;
	}

    if ( server_dll == 0 ) {
        printf( "[unknowncheats] server.dll not found (warning)\n" );
    }

	// Pattern scanning is disabled because we are using dumped offsets directly in offsets.hpp
	// This avoids "invalid signature" errors when patterns change but offsets are known.
...
bool unknowncheats::address_t::initialize( ) {
	...
	printf( "[debug] client_dll: 0x%llX\n", this->game.client_dll );
	printf( "[debug] entity_list: 0x%llX\n", this->game.entity_list );
	printf( "[debug] matrix: 0x%llX\n", this->game.matrix );
