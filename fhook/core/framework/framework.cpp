...
namespace unknowncheats::framework {
	bool render( ) {
        ...
        if ( bRunning ) {
            ...
            // Overlay Focus Logic (Alt-Tab Fix)
            HWND game_window = FindWindowA( NULL, "Counter-Strike 2" );
            ...
            if ( game_window != NULL ) {
                ...
                if ( GetWindowRect( game_window, &game_rect ) ) {
                    // Position overlay to match game window position
                    SetWindowPos( instance, HWND_TOPMOST,
            ...
            // If we are not focused on the game or the menu, make sure we are transparent and don't block input
            if ( foreground_window != game_window && foreground_window != instance ) {
                LONG_PTR windowStyle = GetWindowLongPtr( instance, GWL_EXSTYLE );
                if ( !( windowStyle & WS_EX_TRANSPARENT ) ) {
                    SetWindowLongPtr( instance, GWL_EXSTYLE, windowStyle | WS_EX_TRANSPARENT );
                }
            } else if ( !m_b_open ) {
                 // If we are back in game and menu is closed, ensure we are transparent (redundant safety)
                 LONG_PTR windowStyle = GetWindowLongPtr( instance, GWL_EXSTYLE );
                 if ( !( windowStyle & WS_EX_TRANSPARENT ) ) {
                    SetWindowLongPtr( instance, GWL_EXSTYLE, windowStyle | WS_EX_TRANSPARENT );
                 }
